...
{
    double cost = std::abs(start.PosX() - goal.PosX()) +std::abs(start.PosY() - goal.PosY());
    return cost;
}
...
double HeuristicFunctionDiagnal(Node start, Node goal)
{
    int strait = 1;
    double diagnal = sqrt(2);
    int dx = std::abs(start.PosX()-goal.PosX());
    int dy = std::abs(start.PosY()-goal.PosY());
...
vector<PosNode> AStar(int start_pos[2], int goal_pos[2], int maze[mazeSizeX][mazeSizeY])
{
    ...
    priority_count ++;
    start.priority_stamp = priority_count; // if the priority of two nodes are equal priority queue tends to place the last entered in front(use stamp to assure fifo) 
...
int main()
{
    ...
    build_maze(maze);
    int obstacle_a[2] = {(int)1300/50,(int)1800/50};//<---from camera
    int obstacle_b[2] = {(int)5000/50,(int)5000/50};//<---from camera
    int obstacle_c[2] = {(int)1300/50,(int)1300/50};//<---from camera
    int start_pos[2];
    start_pos[0] = big_to_small_maze(400);//<---my_pos_x
    start_pos[1] = big_to_small_maze(400);//<---my_pos_y
    int goal_pos[2];
    goal_pos[0] = big_to_small_maze(1800);//<---goap_x
    goal_pos[1] = big_to_small_maze(2700);//<---goap_y
